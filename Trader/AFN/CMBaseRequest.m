...
// * const CMNetworkDomain = @"http://101.200.139.156";
 //NSString * const CMNetworkDomain = @"http://www.xiaoban.mobi";
//NSString * const CMNetworkDomain =  @"https://hq.sinajs.cn/";
...
- (void)initHttpSessionManager {
   ...
   //  _manager.responseSerializer = [AFJSONResponseSerializer serializer];
    _manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json",@"application/javascript",@"text/json", @"text/html",@"text/plain", @"text/xml",
                                                                                 @"image/*",nil];
...
+ (instancetype)requestWithSuccessBlock:(CMRequestSuccessBlock)successBlock failureBlock:(CMRequestFailureBlock)failureBlock {
    return [[[self class] alloc] initWithSuccessBlock:successBlock failureBlock:failureBlock];
}
*/
- (void)startCompletionBlockWithSuccess:(CMRequestSuccessBlock)success failure:(CMRequestFailureBlock)failure {
...
- (void)constructURL {
    _url = [NSString stringWithFormat:@"%@%@", _CMNetworkDomain, [self requestURLPath]];
}
...
- (NSURLSessionTask *)sessionTaskForError:(NSError *__autoreleasing *)error {
    CMRequestMethod method = [self requestMethod];

    //请求参数
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithDictionary:[self requestArguments]];

    //此处可以配置所有请求都需要的共同的参数，比如 tCMen
//    NSString *tCMen = @"";
//    if (VALID_STRING([OBGlobal global].userInfo.tCMen)) {
//        tCMen = [OBGlobal global].userInfo.tCMen;
//    }
//    [params setObject:tCMen forKey:kRequestTCMen];
                                           ...
                                           error:(NSError * _Nullable __autoreleasing *)error
{
    ...
    if ([method isEqualToString:@"GET"]) {
        NSLog(@"GET-->url:%@", dataTask.currentRequest.URL.absoluteString);
    } else {
        NSLog(@"POST-->url:%@", dataTask.currentRequest.URL.absoluteString);
    }
...
- (AFHTTPRequestSerializer *)requestSerializer {
    ...
    switch ([self requestSerializerType]) {
        case CMRequestSerializerTypeHTTP: {
            requestSerializer = [AFHTTPRequestSerializer serializer];
        } break;
        case CMRequestSerializerTypeJSON: {
...
- (void)handleRequestResult:(NSURLSessionDataTask *)dataTask
             ...
             responseObject:(id)responseObject
                      error:(NSError *)error
{
   //NSLog(@"response %@responseObject %@",response,(id)responseObject);
///< 当前处于开发模式、模拟器环境下,才显示 HTTP 返回的状态码,方便调试
    ...
    if (error) {
        ...
        NSLog(@"response 78:%@", response);
         [self handleData:responseObject errCode:error.code];
...
- (CMRequestSerializerType)requestSerializerType {
    return CMRequestSerializerTypeHTTP;
}
- (CMResponseSerializerType)responseSerializerType{
    return CMResponseSerializerTypeJSON;
}

- (NSDictionary *)requestArguments {
    NSAssert([self isMemberOfClass:[CMBaseRequest class]], @"子类必须实现requestArguments");
    return nil;
}

- (NSString *)requestURLPath {
    NSAssert([self isMemberOfClass:[CMBaseRequest class]], @"子类必须实现requestURLPath");
    return nil;
}
...
- (void)handleData:(id)data errCode:(NSInteger)errCode {
    NSAssert([self isMemberOfClass:[CMBaseRequest class]], @"子类必须实现handleData:data errCode:errCode");
}
